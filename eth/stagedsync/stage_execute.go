...
=======
	...
	if isPoSa {
		execRs, err = core.ExecuteBlockEphemerallyForBSC(cfg.chainConfig, &vmConfig, getHashFn, cfg.engine, block, stateReader, stateWriter, ChainReaderImpl{config: cfg.chainConfig, tx: tx, blockReader: cfg.blockReader}, getTracer, logger)
>>>>>>> v1.2.5
	} else {
...
<<<<<<< HEAD
	gasState float64, batch ethdb.DbWithPendingMutations) (uint64, uint64, time.Time) {
=======
...
<<<<<<< HEAD
			log.Trace(fmt.Sprintf("un ch st: %x, %d, %x, %x\n", address, incarnation, location, common.Copy(v)))
=======
			logger.Trace(fmt.Sprintf("un ch st: %x, %d, %x, %x\n", address, incarnation, location, common.Copy(v)))
>>>>>>> v1.2.5
