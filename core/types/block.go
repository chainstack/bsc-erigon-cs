...
func (rb RawBody) payloadSize() (payloadSize, txsLen, unclesLen, withdrawalsLen, sidecarsLen int) {
	...
	// size of Sidecars
	if rb.Sidecars != nil {
		sidecarsLen += encodingSizeGeneric(rb.Sidecars)
...
func (bb Body) payloadSize() (payloadSize, txsLen, unclesLen, withdrawalsLen, sidecarsLen int) {
	...
	// size of Sidecars
	if bb.Sidecars != nil {
		sidecarsLen += encodingSizeGeneric(bb.Sidecars)
		payloadSize += rlp2.ListPrefixLen(sidecarsLen) + sidecarsLen
	}
...
func decodeSidecars(appendList *[]*BlobSidecar, s *rlp.Stream) error {
	...
	if _, err = s.List(); err != nil {
		...
		return fmt.Errorf("read BlobSidecar: %w", err)
	}
	for err == nil {
		var sc BlobSidecar
		...
		*appendList = append(*appendList, &sc)
	}
	return checkErrListEnd(s, err)
}
