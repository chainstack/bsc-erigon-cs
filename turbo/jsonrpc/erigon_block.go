...
func (api *ErigonImpl) GetBalanceChangesInBlock(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (map[common.Address]*hexutil.Big, error) {
	...
	latestState, err := rpchelper.CreateStateReader(ctx, tx, blockNrOrHash, 0, api.filters, api.stateCache, api.historyV3(tx), api._blockReader, "")
	if err != nil {
		return nil, err
	}
...
<<<<<<< HEAD:cmd/rpcdaemon/commands/erigon_block.go
	balancesMapping := make(map[common.Address]*hexutil.Big)

	newReader, err := rpchelper.CreateStateReader(ctx, tx, blockNrOrHash, 0, api.filters, api.stateCache, api.historyV3(tx), api._blockReader, "")
	if err != nil {
		return nil, err
	}

=======
>>>>>>> v1.2.5:turbo/jsonrpc/erigon_block.go
